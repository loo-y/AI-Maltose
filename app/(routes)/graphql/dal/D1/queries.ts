...
export const queryUserConversationMessages = async({userId, conversationId}: {userId: string, conversationId: number}) => {
    const requestOptions = {
        ...commonD1Options,
        body: JSON.stringify({
...
export const addMessages = async({userId, userType, conversationId, message}: {userId: string, userType: string, conversationId: number, message: {content: string, contentType: string, createAt: number}[]}) => {
    if(_.isEmpty(message)){
       return null 
    }
    let params: (string | number)[] = [], values: string[] = []
